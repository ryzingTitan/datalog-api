...
class ColumnConfigurationServiceTests {
    ...
    inner class Create {
        ...
        fun `creates correct column configuration from header row`() {
            val actualColumnConfiguration = columnConfigurationService.create(headerRow)
    ...
    private val expectedColumnConfiguration = ColumnConfiguration(
        deviceTime = 0,
        longitude = 1,
        latitude = 2,
        altitude = 3,
    ...
    companion object ColumnConfigurationServiceTestConstants {
        const val headerRow = "Device Time,Longitude,Latitude,Altitude,Air Fuel Ratio(Measured)(:1)," +
